...
class RaspberryPi:
    ...
    def _capture_image_picamera(self, filename: str, attempt_index: int) -> None:
        ...
        attempt_index starts from 1 and is used to adjust exposure/shutter.
        """
        try:
            import picamera
        except ImportError:
        ...
        # Basic heuristic for exposure adjustments across retries
        # First 3 attempts: increase brightness by lengthening shutter and slight positive EV
        # Later attempts: decrease brightness by shortening shutter and negative EV
        if attempt_index <= 3:
            # 10ms, 20ms, 40ms
            shutter_us = min(1000000, 10000 * (2 ** (attempt_index - 1)))
            ev_comp = min(6, 2 * attempt_index)  # up to +6
            iso_value = 200
        else:
            # 10ms, 5ms, 2.5ms ... (brighter scenes)
            factor = max(1, 2 ** (attempt_index - 3))
            shutter_us = max(1000, 10000 // factor)
            ev_comp = -min(6, 2 * (attempt_index - 3))  # down to -6
            iso_value = 100
        ...
        with picamera.PiCamera() as camera:
            ...
            # Allow auto-gain to settle
            time.sleep(0.4)

            camera.shutter_speed = shutter_us
            # Give time for the new shutter to take effect
            time.sleep(0.1)
    ...
    def snap_and_rec(self, obstacle_id_with_signal: str) -> None:
        ...
        while True:
            ...
            # Capture image using picamera with retry-based exposure tweaks
            try:
                self._capture_image_picamera(filename, retry_count)
            except Exception as e:
                self.logger.error(f"Failed to capture image with picamera: {e}")
                # release lock so that bot can continue moving even if capture fails
                self.movement_lock.release()
                try:
                    self.retrylock.release()
                except:
            ...
            # Higher brightness retry heuristic preserved via attempt index
